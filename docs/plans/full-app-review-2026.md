...
**Gap note:** `/api/admin/scripture-sources` is **not** in the SPA by design for now; operators use Bearer JWT + `curl` (see **`docs/deployment/README.md`** → _Admin API authentication_). Optional later: link from admin UI.
...
| Area                                                                                         | Pattern                                                                                                                                                 | Note                                                                                                    |
| -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **Reader chapter** ([`reader-service`](../../server/services/reader-service.ts))             | Aggregates + verse list + in-book prev/next; cross-book prev/next uses **`GROUP BY` + `inArray(book)`** (two queries max)                               | **Implemented (F5):** prior/later books batched instead of per-book sequential aggregates.              |
| **Support emotions** ([`emotion-service`](../../server/services/emotion-service.ts))         | `readEmotionScripturesBySlug`: **one batched** `OR` query for primary-translation verse ranges, then per-row fallback for misses / translation fallback | **Implemented (F4):** batch primary translation; legacy path for incomplete DB hits and fallbacks.      |
...
- **`TutorialPage`** is a lazy **route** and each MDX section is **`React.lazy`**-loaded — Vite emits **separate chunks** per section (see **F6** closed).
...
| --- | --- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1  | P3  | Admin / API | `GET /api/admin/scripture-sources` not used from SPA                                                                                                                                                  | Closed — documented as **ops-only** (`docs/deployment/README.md`, `docs/verse-search-save.md`). Optional UI later.                                                              |
| F2  | P3  | Admin / API | Scripture diagnostics uses **Bearer JWT** (`authMiddleware`); other admin routes use **session + admin role**. Document for operators; align future admin UI or unify auth if desired.                | Closed — **two auth models** documented (`docs/deployment/README.md`, `docs/styleguide/backend-observability-security.md`). Unify auth only if product adds diagnostics to SPA. |
| F3  | P3  | Saved / UX  | Device-scoped saves rely on **secret `x-device-id`**; document threat model (lost device header ≈ access to guest saves).                                                                             | Closed — threat model in **`docs/verse-search-save.md`** + pointer in **`docs/configuration.md`**.                                                                              |

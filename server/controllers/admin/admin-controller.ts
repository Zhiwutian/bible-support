...
import {
  ...
  normalizeAdminPaginationQuery,
} from '@server/lib/validation/index.js';
import {
  listAuthEvents,
  listUsers,
...
export const getAdminUsers = asyncHandler(
  async (req: Request, res: Response) => {
    const parsed = adminPaginationQuerySchema.parse(req.query);
    const normalizedPagination = normalizeAdminPaginationQuery(parsed);
...
export const getAdminAuthEvents = asyncHandler(
  async (req: Request, res: Response) => {
    const parsed = adminPaginationQuerySchema.parse(req.query);
    const normalizedPagination = normalizeAdminPaginationQuery(parsed);
...
export const patchAdminUserRole = asyncHandler(
  async (req: Request, res: Response) => {
